...
<script type="text/javascript">
	...
	$(document).on('click', '#tasksubmit', function(){
		...
		data = JSON.stringify(data.reduce(function(obj, item) { obj[item.name] = item.value; return obj }, {}));
		// console.log(data);
	...
	//删除任務
 	 $("#task_table").on("click",".btn-danger", function() {
 		var data = {categoryId: $(this).attr('del-id') };
 		$.ajax({
	...
	function loadTpl() {
 	...
 	$("#task_table").on("click",".edit_btn", function() {
		...
		getCategoryDown();
		// fetch data
		data = {"categoryId": $(this).attr('edit-id') };
		$.ajax({
		...
		function tianchong(data){
			...
			if(data.categoryImgPcurl && data.categoryImgPcurl.length){
	 		...
	 		$('.categoryProducts.form-control').html((data.categoryProductNames? data.categoryProductNames.split(',').join('<br>') : "暂无产品"));
