...
<div class="main order-info">
	<div class="tab">
		<div class="tab-item active" data-id="all">全部</div>
		<div class="tab-item" data-id="0">未支付</div>
		<div class="tab-item" data-id="1">已支付</div>
		<div class="tab-item" data-id="2">支付失败</div>
		<div class="tab-item" data-id="3">已发货</div>		
	</div>
	...
	$.ajax({
		...
		success:function(result){
			...
			if (!isNaN(parseInt(type))) {
				orderList = orderList.filter(function(item) { return item.orderStatus === parseInt(type);});
			}
			//1、列表数据
...
function toDetailPage(e) {
	window.location.href = '${APP_PATH }/MlfrontOrder/getMlfrontOrderDetailPage?orderId=' + $(e.target).parents('.order-item').data('orderid');
}
