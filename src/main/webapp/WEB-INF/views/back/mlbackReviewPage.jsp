	    ...
	    <div class="main-box">
	        ...
	        <div class="content">
	            <div class="table-box">
	                <!-- operator -->
	                <div class="op">
	                    <a href="#" class="btn btn-default" role="button"> <i class="glyphicon glyphicon-tasks"></i>产品评论列表</a>
...
<script type="text/javascript">
	...
	$('#task_add_modal_btn').click(function(){
		// 获取分类页面模板
		$('.table-box').load('${APP_PATH}/static/tpl/addMail.html');
	});
 	//编辑任务
	  $("#task_table").on("click",".edit_btn", function() {
		...
		$('.table-box').load('${APP_PATH}/static/tpl/addMail.html');
		// fetch data
		data = {"areafreightId": $(this).attr('edit-id') };
		$.ajax({
	...
	$(document).on('click', '#tasksubmit', function(){
		...
		data = JSON.stringify(data.reduce(function(obj, item) { console.log(obj); obj[item.name] = item.value; return obj }, {}));
		console.log(data);
		$.ajax({
	...
	//删除任務
 	 $("#task_table").on("click",".btn-danger", function() {
 		var data = {areafreightId: $(this).attr('del-id') };
 		$.ajax({
