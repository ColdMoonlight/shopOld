...
<script type="text/javascript">
	...
	function to_page(pn){
		$.ajax({
			url:"${APP_PATH}/MlbackEmail/getMlbackEmailByPage",
			data:"pn="+pn,
			type:"GET",
	...
	*/
	function build_task_table(result){
		...
		$.each(task,function(index,item){
            var emailId = $("<td></td>").append(item.emailId);
			var emailName=$("<td></td>").append(item.emailName);
			var emailStatus = $("<td></td>").append((item.emailStatus === 1 ? 'Yes' : 'No'));
			var emailCreatetime = $("<td></td>").append(item.emailCreatetime);
			...
			$("<tr></tr>").append(emailId)
				.append(emailName)
				.append(emailStatus)
				.append(emailCreatetime)
				.append(emailMotifytime)
				.append(btnTd)
	...
	$(document).on('click', '#tasksubmit', function(){
		...
		$.ajax({
			...
			success:function(result){
				if(result.code == 100){
					alert('新建/编辑成功！');
					window.location.href="${APP_PATH}/MlbackEmail/toMlbackEmailPage";
				}
			}
		}); 
	});
	//删除任務
 	$("#task_table").on("click",".btn-danger", function() {
 		var data = {emailId: $(this).attr('del-id') };
 		$.ajax({
			url:"${APP_PATH}/MlbackEmail/delete",
			data: JSON.stringify(data),
			dataType:"json",
			contentType: 'application/json',
			type:"post",
	...
	function tianchong(data){
		...
		$(":input[name='emailName']").val(data.emailName);
 		$(".emailStatus").val(data.emailStatus);
			...
			$.ajax({
				...
				success:function(result){
					...
					if(result.code == 100){
						...
						tianchong(obj);
						$('.summer-note').summernote({focus: true});
					}else{
						alert("联系管理员");
					}
				}
