...
<body>
			...
			<div class="main-box nicescroll">
				...
				<div class="content">
					<div class="table-box">
						<!-- operator -->
						<div class="op">
							<a href="#" class="btn btn-default" role="button"> <i class="glyphicon glyphicon-tasks"></i> 营销渠道列表</a>
	...
	<script type="text/javascript">
		...
		function to_page(pn) {
			$.ajax({
				url: "${APP_PATH}/MlbackChannel/getMlbackChannelByPage",
				data: "pn=" + pn,
				type: "GET",
		...
		function build_task_table(result) {
			...
			$.each(task, function (index, item) {
				...
				var channelId = $("<td></td>").append(item.channelId);
				var channelUname = $("<td></td>").append(item.channelUname);
				var channelNumber = $("<td></td>").append(item.channelNumber);
				var channelCreatetime = $("<td></td>").append(item.channelCreatetime);
				var channelMotifytime = $("<td></td>").append(item.channelMotifytime);
				
				var editBtn = $("<button></button>").addClass("btn btn-primary btn-xs edit_btn")
					.append($("<span></span>").addClass("glyphicon glyphicon-pencil")).append("编辑");
				//为编辑按钮添加一个分类id
				editBtn.attr("edit-id", item.channelId);
				...
				$("<tr></tr>").append(channelId)
					.append(channelUname)
					.append(channelNumber)
					.append(channelCreatetime)
					.append(channelMotifytime)
					.append(btnTd)
		...
		$(document).on('click', '#tasksubmit', function () {
			...
			$.ajax({
				url: "${APP_PATH}/MlbackChannel/save",
				data: data,
				dataType: "json",
				...
				success: function (result) {
					if (result.code == 100) {
						alert('添加成功！');
						window.location.href = "${APP_PATH}/MlbackChannel/toMlbackChannelPage";
					}
				}
			});
		});
		...
		$("#task_table").on("click", ".btn-danger", function () {
			var data = {
					channelId: $(this).attr('del-id')
			};
			$.ajax({
				url: "${APP_PATH}/MlbackChannel/delete",
		...
		function loadTpl() {
			$('.table-box').load('${APP_PATH}/static/tpl/addChannel.html', function () {
				// 设置归属类
			});
		}
		...
		$("#task_table").on("click", ".edit_btn", function () {
			...
			// fetch data
			data = {
				"channelId": $(this).attr('edit-id')
			};
			$.ajax({
				url: "${APP_PATH}/MlbackChannel/getOneMlbackChannelDetail",
				data: data,
				type: "POST",
				success: function (result) {
					if (result.code == 100) {
						obj = result.extend.mlbackChannelOne;
						// console.log(obj);
						tianchong(obj);
					} else {
						alert("联系管理员");
			...
			function tianchong(data) {
				$(":input[name='channelId']").val(data.channelId);
				$(":input[name='channelUname']").val(data.channelUname);
				$(":input[name='channelNumber']").val(data.channelNumber);
				$(":input[name='channelCreatetime']").val(data.channelCreatetime);
				$(":input[name='channelMotifytime']").val(data.channelMotifytime);
			}
		});
