...
$(document).on('click', '#tasksubmit', function(){
	...
	reqData = data.reduce(function(obj, item) { obj[item.name] = item.value; return obj }, {});
	//console.log(reqData)
...
$("#task_table").on("click",".btn-danger", function() {
	var data = {productId: $(this).attr('del-id') };
	$.ajax({
	 		...
	 		if (checkBox.length === 0) {
		 		...
		 		for(var i=0, len=categoryData.length; i<len; i+=1) {
		 			...
		 			html+= '<div class="check-item">' +
		 					'<input type="checkbox" data-id="'+ dataId +'" data-name="'+ categoryData[i].categoryDesc +'" '+ checkedStatus +' class="checkbox">' +
		 					'<label>'+"No."+ categoryData[i].categoryId +"&nbsp;&nbsp;&nbsp;&nbsp;"+ categoryData[i].categoryDesc +'</label>' +
		 				'</div>';
		 		}
		 		html += '</div></div>' +
    ...
    if(productIdUP==null){
        ...
        $.ajax({ 
            ...
            success:function(data){
                ...
                $(self).parent().css({ "background-image": "url("+'${APP_PATH }/static/img/product/'+returl+")" });
            }
        });
    }
...
function addHairStyle(parent, data) {
   ...
   var trHtml = '';
   trHtml += '<td>' +
       '<input class="form-control" data-id="productskuNameDetails" type="text" value="'+ (data.productskuNameDetails ? data.productskuNameDetails : '') +'">'+
     '</td>'+
     '<td>'+
     	'<input class="form-control" data-id="productskuStockDetails" type="text" value="'+ (data.productskuStockDetails ? data.productskuStockDetails : '') +'">'+
     '</td>'+
     '<td>'+
       '<input class="form-control" data-id="productskuMoneyDetails" type="text" value="'+ (data.productskuMoneyDetails ? data.productskuMoneyDetails : '') +'">'+
     '</td>'+
