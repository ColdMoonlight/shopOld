      ...
      <div class="order-item">
        ...
        <div class="list-group">
		   <li class="list-group-item">
		     <div class="group-title"><span>Choose Coupons</span> <span class="price-info"></span><i class="icon bottom"></i></div>
...
<script type="text/javascript">
	...
	var newDate_zsh=year_zsh+"-"+month_zsh+"-"+day_zsh+" "+hours_zsh+":"+minutes_zsh+":"+seconds_zsh; 
...
function renderAddressDetail(parent, data) {
	...
	html += '<div class="address-details address-trigger">' +
	    ...
	    '<div class="address-info">' +
	      ...
	      '</div>' +
	      '<div class="address-i-item">' +
	        '<span class="address-i-address">'+ data.addressDetail + ' ' + data.addressCity + ' ' + data.addressProvince + ' ' + data.addressCountry +'</span>' +
	      '</div>' +
	    '</div>' +
	  '</div>';
	 parent.html(html);
...
function renderAddressAdd(parent) {
	parent.html('<div class="add-address address-trigger"><i class="icon plus"></i> Add address consignee information</div>');
}
/* 初始化地址模块 */
$.ajax({
...
$('.address-modal .cart-submit').on('click', function() {
  ...
  var reqData = formData.reduce(function(obj, item) { obj[item.name] = item.value; return obj }, {});
...
function calAllProductPrice(data) {
	...
	for (var i=0; i<len; i+=1) {
		...
		singlePrice = skuPriceArr.reduce(function(price, item) { return (price + parseFloat(item))}, originalPrice);
		// console.log(data[i])
			...
			success:function(data) {
				...
				for(var i=0; i<len; i+=1) {
					...
					html += '<div class="coupon-item">' +
				        ...
				        '<input type="radio" name="coupon" data-couponid="'+ resData[i].couponId +'" onclick="selectCoupon(event)" class="checkbox">' +
				      '</div>';
				}
			}
		});
...
function checkCouponCode(event) {	
	...
	var data = {couponCode: couponCode2 };	 // MEGA12	couponCode	
	$.ajax({
		url: '${APP_PATH}/MlbackCoupon/getOneMlbackCouponDetailByCode',
		data: JSON.stringify({"couponCode": couponCode2 }),
		type:'post',
		dataType: 'JSON',
		contentType: 'application/json',
		success:function(data) {
