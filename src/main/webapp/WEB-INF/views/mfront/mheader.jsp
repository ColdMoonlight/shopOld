...
<div class="header bd-b">
  <div class="container">
    ...
    <div class="logo">
      <a href="${APP_PATH}/index/isMobileOrPc"><img src="${APP_PATH }/static/m/img/index/logo.png" alt=""></a>
    </div>
...
function renderMainCategory(parent, data) {
  ...
  for(var i in data) {
    if (categoryActiveNum === i) {
      html += '<li class="category-item active" data-id="' + data[i].categoryId + '"><a href="#">'+ data[i].categoryName + '</a></li>';
    } else {
      html += '<li class="category-item" data-id="' + data[i].categoryId + '"><a href="#">'+ data[i].categoryName + '</a></li>';
    }
  }
  parent.html(html);
}
    ...
    if(data.code === 100) {
      categoryData = data.extend.mlbackCategorydownList.reduce(function(obj, item) {
        if(item.categoryParentId === -1) {
          item.list = obj.hasOwnProperty(item.categoryId) && obj[item.categoryId].hasOwnProperty('list') ? obj[item.categoryId].list : [];
