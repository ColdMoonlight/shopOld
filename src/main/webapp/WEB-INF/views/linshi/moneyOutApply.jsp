	    ...
	    <div class="main-box">
	        ...
	        <div class="content">
	            <div class="table-box">
	                <!-- operator -->
	                <div class="op">
	                    <a href="#" class="btn btn-default" role="button"> <i class="glyphicon glyphicon-tasks"></i> 提现申请列表</a>
	                    <a href="${APP_PATH}/HomePage/moneyOutApplySuccess" id="task_add_modal_btn" class="btn btn-primary" role="button" >提现记录</a>
	                </div>
	                <!-- table-content -->
...
<script type="text/javascript">
	...
	function to_page(pn){
		$.ajax({
			url:"${APP_PATH}/moneyOut/moneyOutApplyList",
			data:"pn="+pn,
			type:"POST",
	...
	function build_task_table(result){
		...
		var select = '<select name="moneyoutOperater" class="form-control">'+'<option value ="老关" selected="selected">老关</option>'+'<option value ="少华">少华</option>'+'</select>';
							               
											
		$.each(task,function(index,item){
			//var checkBoxTd = $("<td><input type='checkbox' class='check_item'/></td>");
			var moneyoutIdTd = $("<td></td>").append(item.moneyoutId);
			var moneyoutAccountidTd = $("<td></td>").append(item.moneyoutAccountid);
			 var moneyoutUdidTd = $("<td></td>").append(item.moneyoutUdid); 
			var moneyoutAdminNameTd = $("<td></td>").append(item.moneyoutAdminName);
			var moneyoutSparetwoTd = $("<td></td>").append(item.moneyoutSparetwo);
			var moneyoutPriceOnceTd = $("<td></td>").append(item.moneyoutPriceOnce);
			var moneyoutApplybegintimeTd = $("<td></td>").append(item.moneyoutApplybegintime);
			var moneyoutPayId = $("<td></td>").append(item.moneyoutAlipayname);
			var moneyoutPayName = $("<td></td>").append(item.moneyoutAlipaypeople);
			var moneyoutOperaterTd = $("<td></td>").append(select);
			var moneyoutApplyStateTd = $("<td></td>").append(item.moneyoutApplyState);
			var moneyoutOutingTd = $("<td></td>").append(item.moneyoutOuting);

			var editBtn = $("<button></button>").addClass("btn btn-primary btn-sm edit_btn")
							.append($("<span></span>").addClass("glyphicon glyphicon-pencil")).append("打款");
			//为编辑按钮添加一个自定义的属性，来表示当前员工id
			editBtn.attr("userMsg",JSON.stringify(item));
			var btnTd = $("<td></td>").append(editBtn);
			//var delBtn = 
			//append方法执行完成以后还是返回原来的元素
			$("<tr></tr>").append(moneyoutIdTd)
			    .append(moneyoutAdminNameTd)
				.append(moneyoutPayId) 
				.append(moneyoutPayName)
				.append(moneyoutSparetwoTd)
				.append(moneyoutPriceOnceTd)	
				.append(moneyoutApplybegintimeTd)
				.append(moneyoutOperaterTd)
				.append(btnTd)
	...
	//打款操作
	 $("#task_table").on("click",".edit_btn", function() {
		...
		var json = {"moneyoutId":data.moneyoutId,"moneyoutOperater":$(":input[name='moneyoutOperater']").val(),"moneyoutUdid":data.moneyoutUdid}
		console.log(json);
		$.ajax({
				url:"${APP_PATH}/moneyOut/applyMoneyOutSuccess",
